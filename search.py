...
def breadthFirstSearch(problem):
    ...
    if problem.goalTest(startstate):
        return []

    frontier = util.Queue()
    explored = set()
    frontier.push(Node(startstate, None, None, 0))
    ...
    while not frontier.isEmpty():
        ...
        if node.state not in explored:
            ...
            for action in problem.getActions(node.state):
                ...
                childnode = Node(childstate, node, action, node.path_cost + 1)
...
def aStarSearch(problem, heuristic=nullHeuristic):
   ...
   explored = set()
   while frontier:
       _, currentState, path = heapq.heappop(frontier)
       ...
       if currentState not in explored:
           ...
           for action in problem.getActions(currentState):
               ...
               nextCost = problem.getCost(currentState, action)
               nextPath = path + [action]
               totalCost = nextCost + heuristic(nextState, problem)
               heapq.heappush(frontier, (totalCost, nextState, nextPath))
